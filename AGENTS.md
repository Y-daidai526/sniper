...
receiver/ ROS2 Python
  local-test or match MQTT CustomByteBlock -> 300B data -> seq/loss stats -> PyAV decode -> display
```

`Pacific_doorlock_sniper/` 只作为参考来源。不要修改它，不要把它作为构建目标，不要让 sender/receiver 的主链路依赖它。

## 2. 硬约束

- 顶层只维护新的 `sender/` 和 `receiver/`；不要新增 `compat/`、`common/` 等顶层目录。
...
- 注意：`GenericPublisher` 在某些 ROS2 环境中仍可能需要原 `doorlock_sniper/msg/VideoPacket` 的运行时 type support。缺失时必须只禁用兼容发布，不得影响 0x0310 串口/debug/MQTT 主链路。
...
- 相机基于海康 MVS SDK，发布图像给编码器；相机 topic/frame/曝光/增益/SDK 参数来自 `sender/config/sender_params.yaml`。
...
- 启动时提示用户选择运行模式：
  - local test：连接配置中的本地 broker，默认配置为 `127.0.0.1`，使用 `local_client_id`。
  - match：连接配置中的比赛 broker，默认配置为 `192.168.12.1`，继续提示输入机器人 ID，并将机器人 ID 作为 MQTT client id。
- MQTT broker port/topic/client id 等配置来自 `receiver/config/receiver_params.yaml`。
...
- `hik_camera`：`image_topic`、`frame_id`、`camera_name`、`use_sensor_data_qos`、`acquisition_frame_rate`、`exposure_time`、`gain`、`color_transformation_enable`、`gamma`、`ccm_enable`、`camera_info_url`。
- `video_encoder`：`input_topic`、`video_stream_topic`、`enable_video_stream`、编码参数、预处理参数、`serial_max_rate_hz`、`max_tx_delay_s`、display/debug dump 参数。

`receiver/config/receiver_params.yaml` 只包含 receiver 实际读取的参数：

- MQTT：`local_broker_host`、`match_broker_host`、`broker_port`、`topic`、`local_client_id`、`mqtt_queue_size`。
