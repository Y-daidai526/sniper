...
tools/mqtt_test.py       独立 MQTT seq 诊断脚本
...
local_mqtt 从 stdin 读取 309B 帧时直接写 309。
```
...
MQTT 客户端固定使用 `paho-mqtt 2.1.0` 和 callback API v2，通过 pip 安装。`amqtt` 只用于 sender local_mqtt 内置 MQTT broker。README 里记录了 Debian/Ubuntu externally-managed Python 的处理方式。
...
sender/encoder/video_encoder_node.*     预处理、x264 和编码输出回调
...
sender/local_mqtt/local_mqtt_bridge.*   fork/exec Python local_mqtt
sender/local_mqtt/main.py               stdin 309B frame -> MQTT protobuf
sender/local_mqtt/serial_parser.py      Python 侧 0x0310 校验
sender/local_mqtt/mqtt_server.py        amqtt broker 和 paho publisher
sender/local_mqtt/proto/                CustomByteBlock protobuf
```
...
install/sender/share/sender/local_mqtt/
```
...
相机节点构造结束后，`main.cpp` 会先检查 ROS context；相机初始化失败或等待相机时收到退出信号后，不再继续创建编码器和 executor。
...
LocalMqttBridge：local_mqtt 子进程和 stdin 写入
```

发送线程按 `serial_max_rate_hz` 独立调度，从 buffer 取 299B H264，加 inner seq 并调用 `sniper::protocol::build_frame()` 构造完整 309B 0x0310 帧。编码器与发送线程之间只有一把 buffer mutex 和一个 condition variable，CRC、串口、local_mqtt 和日志均不持有该锁。

local_mqtt 脚本目录不进 config。代码按安装目录查找：

```text
share/sender/local_mqtt
```

开发环境查不到安装目录时，退回：

```text
<当前工作目录>/sender/local_mqtt
```
...
海康相机节点保留 Pacific 的参数声明和 SDK 行为，是本项目配置原则中的明确例外；不要为了统一风格改成 required parameter。
...
GStreamer 任一元素创建、pipeline link 或切换到 PLAYING 失败时，构造函数直接抛异常，不允许留下半初始化节点。`enable_display=false` 时预处理不生成只供四宫格预览使用的中间图像。
...
独立发送线程中 stream_buffer_ 至少有 299B，且 rate gate 允许时：
  ...
  build_frame 后写 SerialWriter 和可选 LocalMqttBridge
```
...
monitor loop 每 500ms 检查一次热插拔。首次打开失败或写入错误关闭 fd 后，即使设备路径没有变化，也会在后续检查中重试。无串口时编码继续跑，stats 里 `port=disconnected`。配置关闭串口时 `port=disabled`。

串口只写完整 309B frame，不写裸 300B data。

### local_mqtt

`local_mqtt` 是本地 MQTT 测试链路，不再使用旧 debug 命名。

C++：

```text
sender/local_mqtt/local_mqtt_bridge.hpp
sender/local_mqtt/local_mqtt_bridge.cpp
```

Python：

```text
sender/local_mqtt/main.py
sender/local_mqtt/serial_parser.py
sender/local_mqtt/mqtt_server.py
```

流程：

```text
LocalMqttBridge fork/exec Python
...
`LocalMqttBridge::write_frame()` 不返回状态；管道错误由 bridge 内部记录并关闭 fd，之后写入为空操作。不要用 `child_pid > 0` 推断子进程仍在运行。

`local_mqtt_start_broker=true` 时，Python 使用 `amqtt` 启动内置 broker。启动失败时尝试连接同地址已有 broker。

local_mqtt 日志只保留异常和状态：

```text
[mqtt] ...
[local_mqtt] stdin EOF ...
[local_mqtt] short read ...
[local_mqtt] parse error ...
```
...
销毁时先停止 ROS poll timer 和 MQTT loop，再结束显示线程。`destroy_node()` 可重复调用；`main()` 使用 `rclpy.try_shutdown()`，显示线程按 `q` 触发 shutdown 后也能安全清理。

### receiver stats

统计窗口 1 秒，直接硬编码在 `StatsTracker.record()` 的判断处。窗口使用 `time.monotonic()`，不受系统时间校准影响。
...
`tools/mqtt_test.py` 是独立诊断脚本，不参与 sender/receiver 启动或安装。它直接订阅 `CustomByteBlock`，解析 300B data 的 inner seq，并打印包间隔、重复包和缺口；连接信息当前直接写在脚本顶部。它使用 paho callback API v2，修改协议或 MQTT 客户端版本时要与主链路一起检查。
...
代码改动后按影响范围运行最小检查。编译验证优先使用两个独立入口；脚本会在构建成功后继续启动节点，确认进入启动阶段后应及时终止：
...
协议改动还必须覆盖合法 309B 帧，以及错误长度、SOF、CRC8、cmd_id、CRC16 的拒绝测试。接口删除或重命名后用 `rg` 确认没有剩余引用，并运行 `git diff --check`。
